...
use ext4_view::{Ext4, Ext4Read, Metadata};
...
impl Ext4Read for BlockBuffer {
    async fn read(&self, start_byte: u64, dst: &mut [u8]) -> core::result::Result<(), Box<dyn Error + Send + Sync + 'static>> {
...
impl Inode for Ext4Inode {
    ...
    async fn create(&self, _name: &str, _file_type: FileType, _permissions: FilePermissions) -> Result<Arc<dyn Inode>> {
        Err(KernelError::NotSupported)
    }
